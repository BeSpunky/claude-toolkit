...
// Conditional blocks (`{{#flag}}...{{/flag}}`) are expanded by the devcontainer generator before writing.
{
  ...
  // Forward the shared-browser noVNC port (6080) in EVERY scaffold. The shared-browser
  // stack (tools/shared-browser, started on demand) serves its noVNC web client here, and
  // 6080 is the ONLY shared-browser port forwarded — CDP 9223 and VNC 5900 stay bound to
  // loopback, so full browser control never leaves the container. Always present, firebase
  // or not; the Firebase dev-server + emulator ports are appended below when applicable.
{{#firebase}}  //
...
{{/firebase}}  "forwardPorts": [6080{{#firebase}}, 4200, 4000, 9099, 8080, 9150, 9199, 5001{{/firebase}}],
