// Retire the FROZEN house sections that pre-0.5.0 projects still carry inline in CLAUDE.md.
//
// WHY THIS LIVES IN A GENERATOR AND NOT ONLY IN A MIGRATION. The retirement has one precondition: a generated
// HOUSE.md must exist, because otherwise the inline copy is the ONLY copy of that guidance and deleting it
// would destroy it. That precondition is satisfied by `house-doc` — which runs LAST in the sequence, after
// the migration ladder. So the 0.25.0 migration could only ever refuse on a project that had no HOUSE.md
// yet, and once `house-doc` gave it one, that migration was already below the project's floor and would
// never be collected again. The cleanup would have been stranded forever, one release after the file it
// needed finally appeared.
//
// Running it from `house-doc`, immediately after HOUSE.md is written, closes that in a single sync: the
// replacement exists by construction, so the refusal branch below is unreachable from here and the check
// costs nothing. It is the narrow exception to "one-way changes are migrations, not healing code in a
// generator" — earned by ORDERING, not convenience: no migration can observe a file a later step creates.
//
// COPIED FROM src/migrations/0.25.0/retire-inline-house-sections.ts, NOT SHARED WITH IT. Migrations are
// self-contained on purpose — someone jumping several versions resolves implementations out of the NEWEST
// package, so a migration that imported this would run today's rules instead of the ones it was written
// against. That migration keeps its own copy and stays frozen; this is the live one.
//
// ── Original rationale, which still explains every rule below ──────────────────────────────────────────────
//
...
export function retireInlineHouseSections(tree: Tree): void {
  ...
  if (removed.length > 0 && !tree.exists(HOUSE_MD)) {
    logger.warn(
      `[house-doc] \`${CLAUDE_MD}\` still carries ${removed.length} inline house section(s) from before ` +
        `0.5.0, but this workspace has no \`${HOUSE_MD}\` — so they are the ONLY copy of that guidance and were ` +
        `LEFT IN PLACE. This should be unreachable when called from \`house-doc\`, which writes ` +
        `\`${HOUSE_RULES_MD}\` and \`${HOUSE_MD}\` immediately before it — so seeing this means those writes ` +
        `did not happen. Re-run the sync; the next one will retire the inline copy once they exist.`
    );
    return;
  }
  ...
  if (removed.length > 0) {
    logger.info(
      `[house-doc] Removed ${removed.length} frozen house section(s) from \`${CLAUDE_MD}\` — ` +
  ...
  if (didRetarget) {
    logger.info(
      `[house-doc] Retargeted reference(s) in \`${CLAUDE_MD}\` from \`${HOUSE_MD}\` to ` +
  ...
  if (keptButSuspicious.length > 0) {
    logger.warn(
      `[house-doc] \`${CLAUDE_MD}\` has ${keptButSuspicious.length} section(s) whose heading matches a ` +
