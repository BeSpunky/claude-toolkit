...
- **1.3 Run the house generators** (Nx-native via devkit `Tree`, format-aware, **all idempotent**):
  ...
  - **Only with `--firebase`:** `nx g @bespunky/nx-tools:firebase-emulators --project=<APP> --workspaceName=<PROJECT>` → writes `firebase.json` (the generator does **not** write `.firebaserc` — that's the Firebase CLI's job; see the "Connect to a real Firebase project" recipe below), generates `apps/<APP>/src/app/firebase.config.ts` (the `provideAppFirebase()` factory using `ngDevMode`), wires `app.config.ts`, adds the `emulators` / `emulators:<svc>` Nx targets (each passes `--project=demo-<PROJECT>` so emulators work without `.firebaserc` / login) while wrapping `serve` to orchestrate emulators + `serve-app` in parallel, AND adds `firebase` + `@angular/fire` to `package.json` (the generator's post-commit `installPackagesTask` runs the install). No shell-side `yarn add`.
