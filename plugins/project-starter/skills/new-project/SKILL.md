...
Paste the returned `firebaseConfig` object into `productionFirebaseConfig` in `apps/<APP>/src/app/firebase.config.ts`. After the backend exists, **App Hosting deploys are GitHub-driven** — pushing to the branch configured at backend-create time triggers a build via Cloud Build and a Cloud Run deploy. The local emulator setup is unaffected by any of this — emulators continue to use the `demo-<workspaceName>` offline project id passed by the Nx targets.
