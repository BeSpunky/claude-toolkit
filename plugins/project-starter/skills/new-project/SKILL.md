...
### Recipe: connect a real Firebase project

The scaffolder deliberately does NOT fabricate `.firebaserc` or the production web config — the cloud state is the Firebase CLI's source of truth, and inventing values would lie about what the user's account actually has and break `firebase deploy` / `firebase use` the moment they're touched. When you (or the user) are ready to wire a real Firebase project, run these from the workspace root inside the devcontainer (where the Firebase CLI is installed):
...
firebase apps:sdkconfig WEB <appId> --project <projectId>        # prints the real web config
```

Paste the returned `firebaseConfig` object into `productionFirebaseConfig` in `apps/<APP>/src/app/firebase.config.ts`. The local emulator setup is unaffected by any of this — emulators continue to use the `demo-<workspaceName>` offline project id passed by the Nx targets.
