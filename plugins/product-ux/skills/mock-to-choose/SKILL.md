...
- **submitted** — sent to Claude: a solid orange pin. Either the user hits **"Submit review (N)"** (batch), or they flip the **auto-send** toggle so each comment submits the instant it's saved (live). This is the "collect then send / send as you save" choice, and it's a per-session toggle.
...
**How Claude receives it — the honest part.** A web page **cannot interrupt an idle Claude**; nothing (not a socket, a hook, or an MCP server) injects a turn into a session that's sitting waiting. So the "live" feel comes from Claude *watching*, two ways:

- **On-demand (default).** The user pins and submits; the rich payload waits on disk. When they say *"go"* (or ping you), read the **inbox** — the submitted-and-not-yet-handled comments — and act:

  ```bash
  curl -s http://127.0.0.1:<port>/state | jq .pending      # submitted & not handled, with dom context
  ...
  Then re-mock upstream and **check each off** (`window.mockHandle(n, {reply})` or `PATCH /comments`). Zero idle cost; one trip to the chat.

- **Live loop (opt-in).** If the user wants continuous back-and-forth, run a self-paced loop (or a `Monitor` on `comments.json`): each tick, process the inbox, mark handled, and reschedule. It *feels* live — handled pins clear from the mock and their replies land in the side-list as green ✓s — at the cost of some tokens while it watches. Offer it; don't impose it.

Either way, the harness stays a plain static-file + HTTP-server tool — no framework, no MCP, no experimental flags. The channel is a file and a status field; the "receive" is a workflow, not a daemon baked into the mock.
...
- **History is a read-only trail.** The Focus side-list carries a **History** timeline (v1 · v2 · v3·current). Open any past round to see its **actual snapshot** (read-only) with that round's comments and your replies; **compare** two rounds side by side (`#diff/<variant>/<vp>/v2`). Deep-linked: `#focus/<variant>/<vp>/v2` reopens a past round; the back button and a shared link honour it.
...
assets/mock-harness/        →  copied into  docs/features/<date>-<slug>/mocks/
  ...
  serve.py  · serve.sh        # random-port server: /comments + /version + hot-reload SSE; prints the URL
...
- **Rounds & history** (see *Rounds*, above) — the mock iterates `v1 → v2…`, comments are version-bound, each committed round is snapshotted, and past rounds are viewable and comparable.
- **A drive + read API** so Claude steers the live page over CDP: `window.mockGoto(name)`, `mockViewport('Phone')`, `mockCompare()`, `mockState()`, `mockCommit(variant, note)`, `mockVersions()`, `mockInbox()`, and deep-link URLs (`#focus/Lantern/Phone`, `#focus/Lantern/Phone/v2`, `#diff/Lantern/Phone/v2`) the back button and a shared link both honour.
