...
- **History is a read-only trail.** The Focus side-list carries a **History** list of committed rounds (each `vN · N comments · note`), and a **"See the evolution →"** link opens the **Evolution** timeline — the whole life of the concept, `v1 → v2 → … → current`, each round a snapshot with the feedback it drew (`#evolution/<variant>/<vp>`, `window.mockEvolution()`). Open any past round to see its **actual snapshot** (read-only) with that round's comments and your replies. Deep-linked: `#focus/<variant>/<vp>/v2` reopens a past round; the back button and a shared link honour it.
