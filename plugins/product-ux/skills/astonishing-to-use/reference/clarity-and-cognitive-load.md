...
- **Disclosure *removes* from the resting frame — it doesn't merely dim.** "Make every label legible at rest" and "calm in one glance" pull against each other; resolve toward **calm**. *Thin* the resting frame — let one thing lead with the rest collapsed or quieted, and reveal the detail on approach, dwell, or tap — rather than keeping every label present-but-dimmed so it *all* still reads at once and the eye must survey to triage. Faint is not absent: a dimmed label is still a label demanding a read. And speak in **one** staged header/eyebrow voice; never stack legibility bands that accrete round after round into competing tiers of always-on text.
