...
- **A glow needs a present hot core — but as the peak of *one* falloff, never a flat overlaid disc.** Two opposite failures bracket this. Too soft: a gentle curve washes out into all-aura-no-centre (no incandescence). Over-corrected: stamping a *separate*, near-white, high-alpha disc on top reads as a flat white *plate* with an edge — unnatural. The truth between them: real incandescence is a bright hot *point* at the very centre that **falls off continuously** into the warm aura. Build it as a *single* gradient whose innermost stop is white-hot but whose brightness drops immediately and smoothly (the white is a point, not a plateau) — not an aura plus a pasted-on core. One curve, hottest at the centre, seamless to the edge.
- **De-band *every* gradient, not just the glows.** Background, atmosphere, and vignette gradients band on dark too (the seam shows as a flat line/edge). Apply the same continuous-falloff + dither everywhere a gradient meets the dark.
- **Depth is not a flat haze gradient.** Faking depth with a single background gradient "wash" both bands and reads as muddy/foggy, not deep. Real depth = **scale** (near big, far small), **occlusion/overlap**, **parallax**, and **atmospheric perspective** (distant things lose contrast + detail and cool) — built from the *elements'* placement and size, not painted on as a backdrop band.
