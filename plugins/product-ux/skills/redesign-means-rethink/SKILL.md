...
> **Related.** This is the **entry gate** to the experience-design trio: a redesign runs `product-ux:envision-the-experience` → `product-ux:stage-the-vision` → `product-ux:realize-the-vision` from scratch. It expresses `engineering:architect-mentality` — *build for the goal, not the brief* (a "redesign" points at a reconceived outcome, not a modified artifact) and *go the extra mile* (the fresh design over the easy reskin) — and pairs with `engineering:architecture-first` for the build half: realize the new design cleanly and remove the old, so the redesign leaves one coherent design, not a patched-over one. It shares its DNA with the toolkit's other anti-anchoring moves — *rearchitect-don't-reskin* (structure) and *re-stage-don't-shrink* (responsive): **rethink-don't-reskin** (the whole design).
