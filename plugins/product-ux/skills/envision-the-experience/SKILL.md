...
A Vision hits **four beats**. They are fixed; *how* you write each one is open — prose, a walk-through, a short film described in words, a letter to the builder, whatever conveys it best. This is guidance, not a form to fill in. Never let the four beats flatten into four boxed headings with a sentence each; that would be the very box-and-text reflex this skill exists to refuse.
...
5. **Walk the arc.** Trace one real person from arrival to departure and make sure the feeling holds and *builds* — fix any moment where the world breaks character or drops back into a generic page.

6. **Write the Vision** in the four beats, in sensory language, naming no implementation. Then hand it to `realize-the-vision`.
