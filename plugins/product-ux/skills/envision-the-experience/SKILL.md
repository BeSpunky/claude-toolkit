...
**Above all, the landing must establish the screen's PURPOSE, PRIMARY ACTION, and HIERARCHY — not just a feeling.** A person arriving must grasp, in a glance and without thinking: *what is this, what is the main thing here, what do I do, what's interactive.* This is hardest and most vital on a **home/landing**, where the temptation is to ship an evocative *mood* (a beautiful place) that never answers "what am I supposed to do." Decide the **primary action** and make it unmistakable; decide what is secondary and let it recede; make every interactive thing legibly interactive (and every non-interactive thing clearly not). And **placement must be purposeful — it must express the hierarchy and function**, never be chosen for visual balance alone: a subject placed off-centre "to balance the layout" reads as *arbitrary* ("why is it there? is it even the focus?") and quietly destroys clarity. If a person can open the screen and not know what it's for or what to do, the world has not landed, however lovely it is. (This is the **dreamy ↔ app-y** contract: beauty without this clarity is half a design — and for a home or any utility surface, the *use* leads, per `astonishing-to-use`.)
...
**Watch utility elements especially — and when one keeps failing, rethink *where it lives*, not just its shape.** Plain wayfinding (a nav, a menu, a tab bar) is the most tempting place to force a world-metaphor and the place it most often reads as **kitsch** — "they tried to be creative and failed." Three moves keep it honest. (1) *Restraint:* navigation can be quietly excellent — elegant, legible, near-conventional, world-native through tone, light, type, and spacing rather than a literal cute shape (a "petal," a "doorway") bolted on by reflex. The mission leads (`astonishing-to-use`): utility wayfinding wants UX-led calm, and the art is spent on the charged moments, not the menu. (2) *Relocate, don't restyle:* if a navigation keeps failing as a forced motif, the fix is usually not the Nth shape — it's that *a separate control is the wrong model here at all*. Interrogate where "the ways you can go" truly live in this world: often they are better **woven into the world's central act or relationship** (you reach a place *through* the heart of the world — the thing the two people do, the fire they tend) than placed as a discrete bar at the edge. The menu was a pointer to "how do I move between the practices," and in a strong world the answer can be an act, not a widget.

**(3) Envision the controls, not just the backdrop — the split-brain failure.** The most seductive version of this trap is the design where the *world* is richly envisioned — an atmospheric backdrop, a living scene, a metaphor that breathes — and the *controls* are conventional furniture (a primary button, a list of rows with chevrons, a tab bar) dropped on top of it and merely **re-skinned** to match the mood: a warmer colour, a softer shadow, a "lit" gradient instead of a flat one, a border removed. That is a **half-envisioned design** — the backdrop went through the one move and the interface never did — and it reads exactly as "atmospheric wallpaper behind an ordinary app." A control is not made world-native by lighting it better; it is made world-native by asking what that action *is* in this world (a reach across the dark is your warmth *travelling* to them — an act your finger performs — not a pill labelled "Reach"). **The controls are inhabitants of the world too — the most important ones, because they are what the hand touches** — so they earn the same interrogation as the scene, not a coat of the scene's paint. The **tell** that this happened: the same control keeps coming back across iterations with the *same* complaint — "still generic," "still dated," "make it more modern," "not creative" — and each fix changes its **finish** (the medium, the reference, the material, the gradient, the border) while its **form** (a button, a list) never moves. **When a finish fix recurs, the form was never envisioned: stop polishing the skin and re-run the one move on the control itself** — its shape is wrong, not its style. And mind the altitude while you do: a control that truly belongs to a *child surface* (its own place in the app) does not get hauled onto the home as a hero button — that collapses a world-you-inhabit into a launcher for one action.
...
- An **envisioned backdrop with un-envisioned furniture** — a richly imagined scene with conventional controls (a primary button, a chevroned list, a tab bar) dropped on top and only *re-skinned* to match the mood (warmer colour, softer shadow, a "lit" gradient, a border removed). Half-envisioned: the scene went through the one move; the controls — the things the hand touches — never did.
- A control **re-skinned across iterations and still landing as generic** — its *finish* changed each round (medium, reference, material, gradient, border) while its *form* (button, list) was never questioned. A recurring finish-fix is the signature of a form that was never envisioned.
