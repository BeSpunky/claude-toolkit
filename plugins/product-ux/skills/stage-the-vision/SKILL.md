...
- **Dated, not modern** — UI rendered in a skeuomorphic, last-decade idiom (heavy bevelled/bordered rounded rects, glossy chrome, drop-shadowed "web 2.0" buttons) when the bar is *contemporary*. Affordances and components should feel current — clean, restrained, well-spaced, of-this-moment — grounded in **today's** great UI (study current award/product work), not a default that reads as old. (And build chrome in the right medium — see `realize`: modern controls are real DOM/CSS, not hand-painted into a canvas.)
