...
- **Dated, not modern — *and generic counts as dated*.** UI in a skeuomorphic, last-decade idiom (heavy bevelled/bordered rects, glossy chrome, "web 2.0" buttons) when the bar is *contemporary* — but also the subtler trap: **a generic *stock* pattern (a default gradient pill, a plain ghost button, a bog-standard card) IS the dated, forgettable default.** "Modern" is not a look you can produce from priors and certify yourself — that path yields the average, which reads as old. Modern means **grounded in SPECIFIC, current, excellent UI** (study *today's* award/product work — real apps people praise *now*) and adapted into something **distinctive for this product**, then judged by an **outside eye**, never self-certified "this is modern." This is `steal-from-great-work` aimed at *interface*, not just art — the model's default UI taste is weak, so lean on real current references. (And build chrome in the right medium — `realize`: modern controls are real DOM/CSS, not hand-painted canvas.)
