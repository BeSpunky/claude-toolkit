...
> **Related.** The mindset beneath it lives in `bespunky-engineering:architect-mentality` (*design for the consumer*, *compensate for weaknesses*, *model the missing concept*, *automate every repeated process*, *go the extra mile*). When the fix requires building a reusable orientation primitive rather than a one-off, that's an `bespunky-engineering:architecture-first` change — design the seam, confirm, then implement.
