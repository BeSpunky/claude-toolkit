...
- `scaffold.sh <project> [app]` — thin launcher; runs the generators on the **local Node** when it's 22.18+, falling back to the `typescript-node` base image via `docker run` only for an old host Node. `--sync [--firebase] <project>` re-applies the house generators idempotently.
...
| Sync / retrofit a project | `… /scaffold.sh --sync [--firebase] <project>` |
