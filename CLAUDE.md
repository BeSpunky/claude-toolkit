...
**`minor` means minor — `adjustSemverBumpsForZeroMajorVersion: false` is set deliberately.** Nx **defaults that to `true`**, which, while a package's major is `0`, silently demotes every relative bump: `major` → `minor`, `minor` → `patch`. Everything here is `0.x`, so on the default `--specifier=minor` on `0.26.4` yields **`0.26.5`** — and the log still reads *"Applied semver relative bump `minor` … to get new version 0.26.5"*, because it echoes the specifier you asked for rather than the one it applied. That line is self-contradictory on its face and has already been misread once as a tooling bug. Nx's reasoning for the default is ordinary (a `0.x` package is pre-stable, so nothing is significant enough to move the second digit), but it disagrees with this repo's own practice: `0.24.0`, `0.25.0` and `0.26.0` were all hand-written second-digit releases from before `nx release` existed here, and under the default you could never *ask* for one with `minor`. So the mapping is turned off and the words mean what they say. Two consequences: `--specifier=major` now really does produce `1.0.0` (only ask for it if you mean it), and an exact version is always available as its own specifier — `nx release version --projects=<p> --specifier=0.27.0`.
...
So `tools/check-release-invariants/check.mjs` asks, after the fact, the question nobody remembers to: *did the thing that should have happened, happen?* It checks that every plugin (and the payload) whose **shipped content** moved since its last release was released again, that the derived registry still agrees with the manifests, and that no migration sits above the payload version. The rule itself — semver ordering and the ceiling — lives once in `tools/check-release-invariants/rules.cjs`, called by the checker, `publish.sh` and the publish workflow; `scaffold.sh` keeps its own copy because it runs inside consumers' projects where this repo does not exist — a cross-reference comment in both is the whole mitigation, deliberately: the only way they can disagree is over prerelease ordering, and this repo has never published a prerelease. It runs in CI on every push (`.github/workflows/release-invariants.yml`) and as a **pre-push hook** (`tools/git-hooks/pre-push`, wired by `core.hooksPath` in `post-create.local.sh`; bypass with `--no-verify`). It **detects and reports — it never bumps**: patch vs minor is a judgment about what a change means to consumers.
...
| Test the migrations | `node tools/test-migrations/run.mjs` — fixture workspaces run through the compiled rungs; idempotence is asserted by the harness for every case (needs `yarn install`; also CI, deliberately **not** the pre-push hook). Add a case: one file in `tools/test-migrations/cases/`, no wiring |
...
- **Every migration gets a fixture case.** `node tools/test-migrations/run.mjs` runs the compiled rungs against `@nx/devkit` `Tree` workspaces built to the shape each one will actually meet. It exists because a migration writes into someone else's repo, once, unattended — and its throwaway ancestor caught five defects in one rung that four reading passes had missed, including a walk that would have edited another git worktree's source. Add the case in the same commit as the rung.
