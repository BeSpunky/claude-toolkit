...
| `voice` | command `voice`, `SessionStart` + `PreToolUse` hooks | **Hear Claude's questions, hands-free.** When Claude asks something, get a **listenable, audio-dedicated summary** — no markup or structure read aloud, just the gist and the options — so you can decide while away from the keyboard. Two triggers: **manual** (`/voice say` reads the pending question on demand; plus `/voice test`, `/voice status`) and **automatic** (`/voice auto on` → a `PreToolUse` hook speaks every `AskUserQuestion` / `ExitPlanMode` the moment it appears; **default off**, so nothing is ever spoken unless you opt in). TTS is a **swappable boundary**: a free `espeak-ng` floor, auto-upgraded to a local **Piper** neural voice when installed (`scripts/install-piper.sh`), or a cloud voice — the plugin owns *when* and *what* to speak, not which engine. Audio reaches the host through the ambient `PULSE_SERVER` (WSLg in the BeSpunky devcontainer). The **speak-half** today; answering **by voice** (STT) is the planned next tier. |
