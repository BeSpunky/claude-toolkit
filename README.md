...
| `voice` | command `voice`, `SessionStart` + `PreToolUse` hooks | **Hear Claude's questions read aloud.** When Claude asks something, get a **listenable, audio-dedicated summary** — no markup or structure read aloud, just the gist and the options — so you can decide while away from the keyboard. Two triggers: **manual** (`/voice say` reads the pending question on demand — including free-text ones; plus `/voice test`, `/voice status`) and **automatic** (`/voice auto on` → a `PreToolUse` hook speaks every structured **multiple-choice question** (`AskUserQuestion`) and **plan approval** (`ExitPlanMode`) the moment it appears; **default off**, so nothing is ever spoken unless you opt in). TTS is a **swappable boundary**: a free `espeak-ng` floor on Linux and `say` on macOS, auto-upgraded to a local **Piper** neural voice when installed (`scripts/install-piper.sh`), and built to allow a cloud voice — the plugin owns *when* and *what* to speak, not which engine. **Runtime:** needs a synthesis engine (`espeak-ng` / macOS `say` / Piper), a player (`paplay`, `aplay`, or `afplay`), `node`, and a reachable audio sink (the ambient `PULSE_SERVER` — WSLg in the BeSpunky devcontainer). After installing, **restart Claude Code once** so the `SessionStart` hook publishes the runtime. The **speak-half** today; answering **by voice** (STT) is the planned next tier. |
