...
| `voice` | command `voice`, `SessionStart` + `PreToolUse` hooks | **Hear Claude's questions and answer by voice.** **Speak:** when Claude asks something, get a **listenable, audio-dedicated summary** — no markup or structure read aloud, just the gist and the options — on demand (`/voice say`, incl. free-text questions; plus `/voice test`, `/voice status`) or **automatically** (`/voice auto on` → a `PreToolUse` hook speaks every **multiple-choice question** (`AskUserQuestion`) and **plan approval** (`ExitPlanMode`) as it appears; **default off**). **Answer:** `/voice answer` records your spoken reply, transcribes it locally, and hands it back as your answer — no typing. TTS is a **swappable boundary** (free `espeak-ng` on Linux / `say` on macOS, auto-upgraded to a local **Piper** neural voice via `scripts/install-piper.sh`, built to allow a cloud voice); STT is **local whisper.cpp**, opt-in via `scripts/install-whisper.sh` (`base.en` → `small.en`), and `listen.sh` boosts the mic itself to beat WSLg's low input gain. **Runtime:** a synthesis engine (`espeak-ng` / `say` / Piper), a player (`paplay`/`aplay`/`afplay`), `node`, a reachable audio sink (the ambient `PULSE_SERVER` — WSLg in the devcontainer), and — to answer — a mic + whisper. After installing, **restart Claude Code once** so the `SessionStart` hook publishes the runtime. Both halves are manual/opt-in today; a **hands-free** mode (Claude asks aloud and waits for your spoken answer, via an MCP tool) is the planned next tier. |
