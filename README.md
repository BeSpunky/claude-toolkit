...
**`bespunky-communication` needs one extra step**, because it ships an **output style** rather than a
skill: installing makes it *available*, not active. Turn it on per project or machine with
`/output-style` → *Pitch to the listener*, or set `"outputStyle": "Pitch to the listener"` in
`settings.json`. Only **one** output style is active at a time — that exclusivity is why the toolkit
never claims the slot for you.
