...
3. Installs `@bespunky/nx-tools` from **npm** as an exactly-pinned devDependency (a real install, not a copy into `node_modules` — see *Migrations* below for why that distinction is load-bearing) and runs the **house generators** — every config change is Nx-native (devkit `Tree`), no hand-rolled file edits. The **per-app** generators (`serve`, `serve-options`, `firebase-emulators`) are not in this list: the `app` generator above already applied them. The workspace-level ones are each gated on the layer they belong to:
   - `nx g @bespunky/nx-tools:devcontainer --name=<project> --nodeMajor=<major>` `[--firebase=true]` → `.devcontainer/devcontainer.json` (Claude CLI/extension, `.claude` persistence, **`CHOKIDAR_USEPOLLING`/`CHOKIDAR_INTERVAL`** for WSL/Docker file-watching, postCreateCommand pre-installing the toolkit plugins; with `--firebase=true`, adds the Firebase CLI + Google Cloud CLI features, the `toba.vsfire` extension, and labeled `portsAttributes` for the emulator suite — but **no explicit `forwardPorts`**: VS Code auto-detects every container binding and forwards each to a free host port, so multiple devcontainers can run in parallel without colliding on the same host port; and with `--voice=true`, writes the **local `voice` feature** into `.devcontainer/features/voice/`, which declares all three halves of the capability itself — the `/mnt/wslg` bind mount, `containerEnv.PULSE_SERVER`, and the `espeak-ng` + `pulseaudio-utils` install — leaving `post-create.sh` only the `bespunky-voice` plugin pre-install, gated on the `BESPUNKY_VOICE` flag the feature exports, so `/voice` speaks the moment the container opens). It also always writes the local **`os-floor`** feature (see *The OS floor* below). Both are referenced by **relative path** (`"./features/os-floor": {}`) — nothing is published to a registry and nothing is downloaded; they ship inside the payload and are versioned by it
...
**Voice support is opt-in (WSL-only).** Pass `--voice` to `scaffold.sh` to make hands-free voice work in the new project's devcontainer. One flag writes **one local devcontainer feature** that owns the whole capability: `.devcontainer/features/voice/` declares the `/mnt/wslg` bind mount bridging WSL2's **WSLg PulseAudio** server, `containerEnv.PULSE_SERVER` pointing at it, and the apt install of the free **espeak-ng** TTS engine + `pulseaudio-utils`. `post-create.sh` keeps only what a build-time step cannot do — pre-installing the **`bespunky-voice`** plugin, which writes into the `/home/node/.claude` bind mount — so `/voice` speaks on first open. The natural neural voice (**Piper**) stays a manual, machine-local upgrade via the plugin's `install-piper.sh`. It's opt-in rather than always-on (unlike Playwright/shared-browser) because the `/mnt/wslg` mount source is **WSL-specific** — binding it on a non-WSL host (macOS, Codespaces) has no source socket. `--sync --voice` retrofits voice onto an existing project too. Never enabled by default.

**Why voice is a feature and not two halves.** It used to be split: the generator hand-placed the mount and the env var in `devcontainer.json`, while `post-create.sh` installed the packages — and, unable to see the `--voice` flag, **inferred** it by testing whether the mount existed (`[ -d /mnt/wslg ]`). That guess was wrong in both directions: any other `/mnt/wslg` read as voice, and a non-WSL audio host got no voice silently. The feature declares mount, env and packages together and exports `BESPUNKY_VOICE=1`, so the one remaining step gates on a **declaration** instead of a side effect.

**The OS floor is always-on, and it installs at BUILD time.** One apt transaction in the local **`os-floor`** devcontainer feature (`.devcontainer/features/os-floor/install.sh`, written by the generator and referenced as `"./features/os-floor": {}`) installs the packages *every* BeSpunky devcontainer carries, whatever it is a devcontainer for — gated on no flag at all. Three groups: the **shared browser**'s stack (`xvfb`, `x11vnc`, `novnc`, `websockify`, `fluxbox`, fonts), **`tmux`**, and the utilities the house tooling shells out to (`iproute2`, `procps` for the `sysctl` the worktree-domains proxy needs, `curl`). `tmux` is there so an interactive shell opened into the container **from outside** can be durable: the Docker Engine API has no endpoint to re-attach to an existing `exec`, so a bare exec's *process* survives its opener restarting but its *terminal* does not — inside a tmux session that costs a redraw instead. No tmux config is written on purpose (a house default set with `set-option -g` would apply server-wide, to whatever the human is running in that container too) — presence on `PATH` is the whole contract, and callers pass their own options. Best-effort with retry, like every other apt step: a flaky mirror warns and prints the exact command to finish by hand — and here it **exits 0 rather than failing the build**, because a feature's non-zero exit leaves the developer with no container at all, usually over a transient DNS blip.

**Why the floor is a feature and the rest of `post-create.sh` is not.** A feature runs during the **build**; several mounts attach at **start** and cover whatever the build put underneath them. So the test is *does a mount replace this path?* — the apt floor installs into ordinary system paths and moves; the Claude plugin pre-install (`/home/node/.claude`, a bind mount), the ~150 MB Playwright Chromium download (the `ms-playwright` volume) and the two ownership fixes on volume mount points all stay. The payoff is a case that was **silently broken**: where a project already had its own devcontainer, the generator writes the house script as `post-create.bespunky.sh` and never chains it, so the floor never installed and nothing said so — whereas an adopted `devcontainer.json` gets a missing `features` sub-key **merged in**, so a feature installs there by construction. Build layers are cached too, so rebuilds stop re-running apt. The shared browser's *packages* travel with the floor; the **capability** does not — its `runArgs` (`--sysctl`, `--add-host`) have no feature equivalent and its noVNC band is single-sourced from `novnc-band.ts`.
