...
Installing the `engineering` and `workflow` plugins makes the `architect-mentality`,
`architecture-first`, and `feature-package` skills available — but a skill only fires
...
2. **The skills** — the depth: `architect-mentality` (the full architect mindset), `architecture-first` (the loop, the patch smells to refuse, the redesign moves), `redesign-means-rethink` (the entry gate to the experience-design trio), and `feature-package` (one effort, one slug, one folder — where every durable artifact lives, and what may be thrown away).
...
the plugins should paste the canonical directives below into their `CLAUDE.md`, so the mindset, the rule, and the
redesign discipline are always in context — each is an always-on directive
