...
The `serve-with-shared-browser` target and the separate `serve-worktree` executor named throughout this record have since been **folded into one public `@bespunky/nx-tools:serve` executor**. The current surface (canonical in the scaffolded project's generated `HOUSE.md` and in `bespunky-browser-automation:shared-browser`):
...
## Update — the noVNC port is ALLOCATED (supersedes every "`6080` is forwarded" statement below)

The fixed `6080` this record specifies was correct for one devcontainer and wrong for two. The failure is not a bind error — it's a **silent wrong target**: each container's netns has its own free `6080`, so `up` succeeds in both; the editor then forwards the second container's to a *different* host port; and every URL that names `6080` (this script's, the executor's, the docs') points at the **first** container's browser. The human verifies the wrong app and nothing anywhere errors.

The fix removes the assumption rather than handling the conflict — the port is a **result**, not a constant:

- **`forwardPorts` no longer lists the noVNC port.** It is allocated at `up` from the `6080-6119` band and auto-forwarded. The editor prefers host port == container port and only remaps on conflict, so allocating a number nobody else holds makes the forward an identity mapping — which is the only way a `localhost:<port>` URL printed *inside* a container can be true, since nothing in-container can read the editor's chosen host port.
- **Three gates, of unequal strength — say so.** (1) A **cross-container registry**: a claim file per port in `bespunky-shared-ports`, a *fixed-name* docker volume every BeSpunky devcontainer mounts (`chmod 1777`, because ownership in a shared volume is a UID and containers may differ). This is the gate that carries the guarantee. (2) **Container-local** `port_listening` — a sanity check only; it can never see the collision, which is the whole point. (3) A **host probe** over the gateway — it sees host listeners on the all-interfaces address, but a listener on the host's own `127.0.0.1` (what the editor's forwards are) may be invisible, and on a plain Linux bridge usually is. So gate 3 can only ever *reject* a port; it is **not** proof of host-side freeness.
- **`requireLocalPort` is the backstop, and it dictates the config shape.** Whatever gets past all three gates (a host-loopback squatter, a second Docker engine with its own registry) is caught by the editor itself: `requireLocalPort: true` turns a silent host-side remap into a prompt. It is honoured **only on exact numeric `portsAttributes` keys** — a `"6080-6119"` range key matches for `label`/`onAutoForward` but silently discards it. Hence one entry per band port, emitted by the generator from `novnc-band.ts`, the single module both generators read so the script's band and the editor's coverage cannot drift.
- **Identity is the DEV CONTAINER — `${devcontainerId}`.** Not the tree (one browser per container; a per-tree key would allocate a second port for a browser that already exists) and not the container *instance* (the hostname is the instance id, so keying on it burns a fresh slot on every rebuild, orphans the old claim for a TTL, and exhausts a 20-slot band in ~20 rebuilds). `${devcontainerId}` is documented as *"unique to the dev container … and stable across rebuilds"*, which is exactly the lifetime a port reservation wants. A container reclaims its own recorded claim **before** walking the band, so the URL is a property of the container rather than of who else happens to be running; and `up` GCs any other claim in its own name.
- **The LIVE LISTENER outranks every record.** "The port we allocated" and "the port that is live" are different facts. Conflating them had a reachable failure: on the upgrade path a `--sync` rewrites the script while a stack from the old fixed-port version is still up — live websockify, no recorded port — so trusting the record allocated a *second* port, skipped spawning websockify ("already running"), failed the readiness gate, and left `url` printing a port nothing was ever bound to, **with exit 0**, wedged until someone ran `down`. So resolution now asks the running websockify which port it bound (cmdline, falling back to the kernel's socket table) and self-heals the record from it; a conflicting `SB_WEB` pin is reported and ignored rather than wedging. A claim is never GC'd for a port something is actually serving on.

- **Claims are TTL'd (7d, refreshed on every `up`)**, so a deleted container's port recycles with no cleanup step. `down` keeps the claim (it's the container's reservation, not the process's); only `clean` releases it. An explicit `SB_WEB` pin still records a claim — the escape hatch must not make a container invisible to its neighbours.
- **One source of truth for the URL, and it refuses to guess.** `shared-browser url` dies with "no noVNC port allocated yet" rather than printing a speculative port; `status --json` exposes `webPortAllocated`, plus `registry` / `hostProbe` / `stableIdentity` so a caller can tell a verified allocation from an educated guess. `up` warns on stdout when a gate is unavailable. The `serve` executor **asks** the CLI and falls back to an instruction, never a composed URL.

### Where the arbitration lives, and how it is validated

- **`tools/shared-browser/port-claim.mjs` owns arbitration; the bash CLI owns process supervision.** Bash is the right material for `ss`/`/proc`/`setsid`/PID lifecycle; it is the wrong material for a registry with ownership, expiry and atomicity. The cost was concrete: the bash version's `[ -f claim ] && write` needed an flock and, on lock timeout, proceeded **unlocked** — a real double-booking race. Node's `open(path,'wx')` is atomic create-if-absent, so the race is gone by construction and there is no lock to time out. The bash side keeps the one thing only it can know: **which port is live right now**.
- **The registry is never created, only used.** A directory the process made itself would be container-local — it would look perfectly usable while giving zero cross-container guarantee. Absent means absent, and `up` says so.
- **`port-claim.test.mjs` is the validation a bash implementation could not have.** 15 cases including a genuine **multi-process race** (N containers allocating against one registry concurrently, real sockets, real files) and the same-container-racing-itself case. `node --test tools/shared-browser/`.
- **`tools/validate-shared-browser/two-container-check.sh`** covers what no in-process test can: two *real* containers on a *real* shared volume — distinct ports, self-owned claims, a rebuild reclaiming its port, honest reporting when the volume is absent, and a live port surviving a neighbour's GC. It needs a Docker engine and refuses clearly without one.
- **The mapping is PROVEN, not assumed.** After websockify binds, `up` writes a random token into the noVNC webroot and fetches it back *through the host gateway*. Our token back = proof; different content = a caught wrong-target, reported loudly; no route to the host = reported as `unverified` rather than assumed good. Exposed as `hostVerified` in `status --json`.
- **`requireLocalPort: true`** on one exact `portsAttributes` key per band port (a range key silently discards the flag) makes any host-side remap prompt the human instead of happening quietly.
- **The tab tells the truth.** `x11vnc -desktop "<project> (shared browser)"` puts the project name in the browser tab title. Ports are something you must reason about; a tab naming the wrong project is something you *see* — and it is the one check that survives a dismissed prompt, a blind probe, a second Docker engine, or a URL typed from memory.

### Single-owner host ports (`:80`, `:4200`) — arbitrated for ATTRIBUTION

`<slug>.localhost` has no port to remap and `:4200` is the only origin registered for real Google OAuth, so neither can move; with two devcontainers up, both silently belong to whichever started first. They now go through the **same registry**, via `port-claim.mjs advise`: first container wins, and every other container is **told which one holds it** — `worktree-domains` on proxy start, and the `serve` executor for `:4200`. Nothing fails and nothing changes hands; the deliverable is that the loser learns the owner instead of debugging a mystery. For a resource with genuinely one slot, attribution is the whole available fix.
...
- **`shared-browser` generator (nx-tools):** writes `tools/shared-browser/*`; wires the workspace `shared-browser` lifecycle project; adds a `/.shared-browser` `.gitignore` entry (covers a relocated in-workspace `SB_RUNTIME`). The **app generator** wires the per-app `serve-with-shared-browser` target. Runs for every scaffold (no flag) and is `--sync`-idempotent. *(Follow-up: a self-extinguishing devcontainer welcome-banner line for discoverability — deferred to v2; for now the printed noVNC URL, the `:6080` forward toast, and the skill carry discovery.)*

---

## Layer 4 — the skill (`bespunky-browser-automation:shared-browser`)

**Decision tree (the reconciliation — prevents tool sprawl):**
| Situation | Tool |
| --- | --- |
| Pure automated check, no human watching, headless is fine | `bespunky-browser-automation:playwright` (headless script) |
...
4. **`--sync` for existing projects** — the generator + devcontainer/post-create edits back-fill on `scaffold.sh --sync` (same as every other house capability).
