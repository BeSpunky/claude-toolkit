...
- **The LIVE LISTENER outranks every record.** "The port we allocated" and "the port that is live" are different facts. Conflating them had a reachable failure: on the upgrade path a `--repair` rewrites the script while a stack from the old fixed-port version is still up — live websockify, no recorded port — so trusting the record allocated a *second* port, skipped spawning websockify ("already running"), failed the readiness gate, and left `url` printing a port nothing was ever bound to, **with exit 0**, wedged until someone ran `down`. So resolution now asks the running websockify which port it bound (cmdline, falling back to the kernel's socket table) and self-heals the record from it; a conflicting `SB_WEB` pin is reported and ignored rather than wedging. A claim is never GC'd for a port something is actually serving on.
